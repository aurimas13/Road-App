...
# query_result = db.session.query(BatchUpdate, Weather, Traffic).join(
#     Weather, BatchUpdate.batchId == Weather.batchId, isouter=True).join(
#     Traffic, BatchUpdate.batchId == Traffic.batchId, isouter=True).all()
# print(query_result)
# batch_date = query_result['BatchUpdateDate'] else if query_result['BatchUpdateDate'] is not None else "1970-01-01 00:00"
